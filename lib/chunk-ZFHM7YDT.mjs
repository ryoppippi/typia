...
(function(NotationGeneralProgrammer2) {
  NotationGeneralProgrammer2.returnType = (rename) => (type) => `typia.${StringUtil_exports.capitalize(rename.name)}Case<${type}>`;
  ...
  const decode_array_inline = /* @__PURE__ */ __name((config) => (importer) => (input, array, explore) => FeatureProgrammer.decode_array(config)(importer)(NotationJoiner.array)(input, array, explore), "decode_array_inline");
  ...
  const decode_native = /* @__PURE__ */ __name((type) => (input) => type === "Date" ? ts.factory.createNewExpression(ts.factory.createIdentifier(type), void 0, [
    input
  ]) : input, "decode_native");
  ...
  const PREFIX = "$c";
  const configure = /* @__PURE__ */ __name((rename) => (project) => (importer) => {
    const config = {
      types: {
        ...
        output: /* @__PURE__ */ __name((type, name) => ts.factory.createTypeReferenceNode(NotationGeneralProgrammer2.returnType(rename)(name ?? TypeFactory.getFullName(project.checker)(type))), "output")
      },
      prefix: PREFIX,
      trace: false,
      path: false,
...
//# sourceMappingURL=chunk-ZFHM7YDT.mjs.map