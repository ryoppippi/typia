 ...
 * @internal
 */
export const stringify_regular_properties = (p: {
  regular: IExpressionEntry<ts.Expression>[];
  dynamic: IExpressionEntry<ts.Expression>[];
}): ts.Expression[] => {
  const output: ts.Expression[] = [];

  p.regular.sort((x, y) => sequence(x.meta) - sequence(y.meta));
  p.regular.forEach((entry, index) => {
    // BASE ELEMENTS
    ...
    if (index !== p.regular.length - 1 || p.dynamic.length !== 0)
